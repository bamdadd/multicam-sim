"""Left-to-right handoff scene: an object crosses the space, camera to camera.

A domain-neutral scenario built to show a moving object being handed off across
overlapping cameras as it travels in a straight line from left to right (world
``-x -> +x``):

  * a **left** camera frames the left of the space: the object starts fully in
    its view and gradually leaves it as it moves right;
  * two **right** cameras frame the middle and right of the space with
    overlapping fields of view, so as the object leaves the left camera it
  ...
  * a **wide** camera stands well back and frames the whole line, so it sees the
    object on EVERY frame — the always-on reference view.
...
true to false as the object exits, the two right cameras' rise as it enters (with
an overlap band where both are true), and the wide camera is true throughout. The
object keeps ONE stable ``entity.id``, the cross-camera identity a tracker must
...
#: The object travels this straight line, left to right, at torso height.
_START_X = -8.0
_END_X = 8.0
_OBJECT_Y = 0.0
_OBJECT_Z = 1.0
...
def _station_views() -> list[StationView]:
    ...
    """
    return [
        # Left camera: frames the left of the line; the object starts here.
        StationView(position=(-5.0, _CAM_Y, _CAM_Z), look_at=(-5.0, 0.0, _OBJECT_Z), fov_deg=70.0),
        # Middle-right camera: frames the centre; picks the object up while it is
        # still in the left camera (an overlap band, no blind gap), overlapping
        # both neighbours.
        StationView(position=(0.0, _CAM_Y, _CAM_Z), look_at=(0.0, 0.0, _OBJECT_Z), fov_deg=75.0),
        # Right camera: frames the right end; overlaps the middle-right camera in
        # a band, then holds the object to the end.
        StationView(position=(6.0, _CAM_Y, _CAM_Z), look_at=(6.0, 0.0, _OBJECT_Z), fov_deg=60.0),
        ...
        StationView(
            ...
            look_at=(_START_X, 0.0, _OBJECT_Z),
            fov_deg=90.0,
        ),
    ]
...
def build_handoff_ltr_scene() -> Scene:
    ...
    start = (_START_X, _OBJECT_Y, _OBJECT_Z)
    end = (_END_X, _OBJECT_Y, _OBJECT_Z)
    frames = Path.linear(start, end).compile_frames(_FPS, _NUM_FRAMES, name="center")
    entity = Entity(id=OBJECT_ID, frames=frames)
