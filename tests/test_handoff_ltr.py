...
One parcel crosses the space in a straight line (world ``-x -> +x``) under four
...
down the length of the line toward the far left, that sees the parcel on EVERY
...
def test_parcel_moves_left_to_right(tmp_path: Path) -> None:
    """The parcel's world x is monotonically non-decreasing across the frames."""
    manifest = _load(tmp_path)
    xs = [f["points"]["center"]["xyz_gt"][0] for f in _frames(manifest)]
    assert xs[0] < xs[-1], "parcel should travel toward +x"
    assert all(b >= a for a, b in zip(xs[:-1], xs[1:], strict=True))


def test_wide_camera_sees_parcel_every_frame(tmp_path: Path) -> None:
    """The far-right wide camera (looking down the line to the far left) is the
    always-on reference: the parcel is in_view on it on EVERY frame."""
    manifest = _load(tmp_path)
    frames = _frames(manifest)
    assert all(_in_view(f)[CAM_WIDE] for f in frames)


def test_left_camera_sees_then_loses_the_parcel(tmp_path: Path) -> None:
    """The LEFT camera sees the parcel at the start and loses it once the parcel
    has moved right (a true->false transition, and it does not come back)."""
    manifest = _load(tmp_path)
    seen = [_in_view(f)[CAM_LEFT] for f in _frames(manifest)]
    assert seen[0], "left camera should see the parcel at the start"
    assert not seen[-1], "left camera should have lost the parcel by the end"
...
def test_right_cameras_pick_the_parcel_up(tmp_path: Path) -> None:
    """The two RIGHT cameras enter after the start (the parcel is NOT in either at
    ...
    assert not first[CAM_MID_RIGHT] and not first[CAM_RIGHT], (
        "right cameras should not yet see the parcel at the start"
    )
    assert last[CAM_RIGHT], "the right camera should hold the parcel at the end"
...
def test_handoff_has_overlap_and_no_near_camera_blind_gap(tmp_path: Path) -> None:
    ...
    the wide reference) see the parcel at once (overlap bands), and there is never
...
def test_stable_identity(tmp_path: Path) -> None:
    """One parcel with a stable id across every frame."""
    manifest = _load(tmp_path)
    assert len(manifest["entities"]) == 1
    assert manifest["entities"][0]["id"] == PARCEL_ID
